...
while (true)
{
    ...
    switch (choice)
    {
        ...
        case "7":
            RevenueAtRiskDashboard();   // ✅ ADD THIS
            break;
...
static void RevenueAtRiskDashboard()
{
    ...
    // ✅ GROUP BY in SQL (most important part)
    var summary = db.Claims
        ...
        .Select(g => new ClaimSummaryDto
        {
            Status = g.Key,
            ClaimCount = g.Count(),
            TotalBilled = g.Sum(c => (decimal?)c.BilledAmount) ?? 0m,
            TotalReimbursed = g.Sum(c => (decimal?)c.ReimbursedAmt) ?? 0m,
            Gap = g.Sum(c => ((decimal?)c.BilledAmount ?? 0m) - ((decimal?)c.ReimbursedAmt ?? 0m))
        })
    ...
    // ✅ Display
    Console.WriteLine($"{"Status",-12} {"Claims",-10} {"Billed",-15} {"Reimbursed",-15} {"Gap"}");

    foreach (var item in summary)
    {
        Console.WriteLine($"{item.Status,-12} {item.ClaimCount,-10} {item.TotalBilled,-15:F2} {item.TotalReimbursed,-15:F2} {item.Gap:F2}");
    }
