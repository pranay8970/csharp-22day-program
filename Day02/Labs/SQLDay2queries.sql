...
select EncounterType,AdmitDate from Encounter order by Admitdate; 
...
select p.FullName as ProviderName,d.Name as DepartmentName,count(e.EncounterId) as TotalEncountersHandled,
RANK() OVER (ORDER BY COUNT(e.EncounterId) desc) as ProviderRank from Encounter e INNER JOIN Provider p on e.ProviderId = p.ProviderId
INNER JOIN Department d on e.DepartmentId = d.DepartmentId group by p.FullName, d.Name order by TotalEncountersHandled desc;
...
--SET @AsOfDate = DATEADD(MONTH, -6, SYSUTCDATETIME());
...
FROM Insurance
FOR SYSTEM_TIME ALL
WHERE PatientId = 1 ORDER BY ValidFrom;
